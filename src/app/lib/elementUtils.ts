...
export const getDirectWikipediaImageUrl = (url: string | null): string | null => {
  if (!url) return null;

  // If it's already a direct upload link, return it as-is
  if (url.includes("upload.wikimedia.org")) {
    return url;
  }

  // Convert Wikipedia File page to direct upload URL
  if (url.includes("wikipedia.org/wiki/File:") || url.includes("wikimedia.org/wiki/File:")) {
    const filenamePart = url.split("File:")[1];
    if (!filenamePart) return url;

    // Replace spaces with underscores and decode URI characters
    const filename = decodeURIComponent(filenamePart).replace(/ /g, "_");

    // Compute MD5 of filename to get Wiki's shard folders
    const hash = md5(filename);
    const char1 = hash.substring(0, 1);
    const char2 = hash.substring(0, 2);

    return `https://upload.wikimedia.org/wikipedia/commons/${char1}/${char2}/${filename}`;
  }
